<template>
  <div>
    ...
    <div class="tool-bar mt-4">
      ...
      <a-tooltip title="缩小">
        <a-button type="text" :icon="h(ZoomOutOutlined)" @click="graphZoom(-0.2)" />
      </a-tooltip>
    </div>
...
import { ZoomInOutlined, ZoomOutOutlined, PlusSquareOutlined } from '@ant-design/icons-vue'
...
const currentWorkflowInstance = ref({})
const showEditPaneRef = ref()
const showEditPane = ref(false)
const graphHolder = ref({})
...
const prettyLayout = async (graph) => {
  // 1. 获取 Graph 实例中的数据
...
const { run: pollingProgress, cancel: cancelPolling } = useRequest(
  async (params) => {
